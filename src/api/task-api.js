...
export class TaskAPI extends BaseAPI {
  ...
  async searchTasks(query, options = {}) {
    const { fields = ['name', 'description'], limit = 50 } = options;
  ...
  async getTaskStats() {
    ...
    const recentCreated = db
      .prepare('SELECT COUNT(*) as count FROM tasks WHERE created_at >= ?')
      .get(weekAgo);
    stats.recent.created = recentCreated.count;

    const recentUpdated = db
      .prepare('SELECT COUNT(*) as count FROM tasks WHERE updated_at >= ? AND created_at < ?')
      .get(weekAgo, weekAgo);
    stats.recent.updated = recentUpdated.count;

    const recentCompleted = db
      .prepare('SELECT COUNT(*) as count FROM tasks WHERE completed_at >= ?')
      .get(weekAgo);
  ...
  getTaskByStringId(db, taskStringId) {
    if (!validateTaskId(taskStringId)) {
      throw new ValidationError('Invalid task ID format', [
        `Task ID must match pattern TASK-XXX where XXX is a number`,
      ]);
    }
  ...
  enrichTask(db, task) {
    ...
    const keywordsStmt = db.prepare(
      'SELECT keyword, keyword_type FROM task_keywords WHERE task_id = ?'
    );
