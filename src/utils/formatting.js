...
export function formatTask(task, options = {}) {
    ...
    if (showDetails) {
        ...
        if (task.dependencies && task.dependencies.length > 0) {
            lines.push(indentStr + '  ' + chalk.gray(`Depends on: ${task.dependencies.map(d => d.depends_on_name).join(', ')}`));
        }

        if (task.dependents && task.dependents.length > 0) {
            lines.push(indentStr + '  ' + chalk.gray(`Blocks: ${task.dependents.map(d => d.dependent_name).join(', ')}`));
        }
