...
describe('Dependency Manager', () => {
  ...
  describe('Dependency Impact Analysis', () => {
    beforeEach(() => {
      ...
      db.prepare(
        ...
        testTasks.B,
        testTasks.A,
        'blocks', // B is blocked by A
        testTasks.C,
        testTasks.A,
        'requires', // C depends on A
        testTasks.D,
        testTasks.A,
        'requires' // D is subtask of A
      );
    });
    ...
    test('should calculate impact when task completes', () => {
      ...
      const unblockedTask = impact.affected_tasks.find(
        task => task.id === testTasks.B && task.impact_type === 'unblocked'
      );
      assert.ok(unblockedTask);

      // Should suggest unblocking B
      const unblockAction = impact.recommended_actions.find(action => action.includes('Unblock'));
    ...
    test('should identify validation errors for invalid transitions', () => {
      ...
      // const _hasSubtaskError = impact.validation_errors.some(error =>
    ...
    test('should handle archiving with active dependents', () => {
      ...
      // const _hasActiveDependentError = impact.validation_errors.some(error =>
    ...
    test('should suggest parent progress update for subtask completion', () => {
      ...
      const parentUpdateTask = impact.affected_tasks.find(
        task => task.impact_type === 'parent_progress_update'
      );
  ...
  describe('Status Transition Validation', () => {
    ...
    test('should validate completion with blocking dependencies', () => {
      ...
      const hasBlockingError = impact.validation_errors.some(error => error.includes('blocked by'));
    ...
    test('should validate completion with incomplete subtasks', () => {
      ...
      db.prepare('DELETE FROM task_dependencies WHERE task_id = ? AND depends_on_id = ?').run(
        testTasks.C,
        testTasks.B
      );
      db.prepare(
      ...
      const hasSubtaskError = impact.validation_errors.some(
        error => error.includes('subtask') && error.includes('Task D')
      );
    ...
    beforeEach(() => {
      ...
      db.prepare(
        `
                ...
                VALUES (?, ?, ?), (?, ?, ?)
            `
      ).run(testTasks.B, testTasks.A, 'blocks', testTasks.C, testTasks.B, 'requires');
    });
  ...
  describe('Graph Optimization', () => {
    beforeEach(() => {
      ...
      db.prepare(
        ...
        testTasks.C,
        testTasks.A,
        'blocks', // Redundant: A -> C (already through B)
        testTasks.D,
        testTasks.A,
        'blocks' // A blocks multiple tasks
      );
    });
    ...
    test('should identify redundant dependencies', () => {
      ...
      const redundantDep = redundant.find(
        dep => dep.from_id === testTasks.C && dep.to_id === testTasks.A
      );
    ...
    test('should calculate hierarchy depth', () => {
      ...
      db.prepare(
        `
                ...
                VALUES (?, ?, ?), (?, ?, ?)
            `
      ).run(testTasks.B, testTasks.A, 'requires', testTasks.C, testTasks.B, 'requires');
    ...
    test('should get descendants correctly', () => {
      ...
      db.prepare(
        `
                ...
                VALUES (?, ?, ?), (?, ?, ?)
            `
      ).run(testTasks.B, testTasks.A, 'requires', testTasks.C, testTasks.A, 'requires');
