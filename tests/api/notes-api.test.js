...
describe('Notes API', () => {
  ...
  describe('Note Creation', () => {
    test('should add a comment note', async () => {
      const note = await notesAPI.addNote(testTask.id, 'This is a test comment', 'comment', {
    ...
    test('should add a system note', async () => {
      const note = await notesAPI.addNote(testTask.id, 'Task status changed', 'system');
    ...
    test('should validate note types', async () => {
      await assert.rejects(
        async () => await notesAPI.addNote(testTask.id, 'Invalid note', 'invalid_type'),
        ValidationError
      );
    });
    ...
    test('should require note content', async () => {
      await assert.rejects(
        async () => await notesAPI.addNote(testTask.id, '', 'comment'),
        ValidationError
      );
    });
    ...
    test('should validate task existence', async () => {
      await assert.rejects(
        async () => await notesAPI.addNote(99999, 'Note for non-existent task', 'comment'),
        APIError
      );
    });
  ...
  describe('Note Retrieval', () => {
    beforeEach(async () => {
      ...
      await notesAPI.addNote(testTask.id, 'Decision made', 'decision', {
  ...
  describe('Note Updates', () => {
    ...
    test('should handle non-existent note', async () => {
      await assert.rejects(
        async () =>
          await notesAPI.updateNote(99999, {
            note: 'Updated',
          }),
        APIError
      );
    ...
    test('should record update in timeline', async () => {
      ...
      const updateNote = task.recent_notes.find(
        note => note.note_type === 'state_change' && note.note.includes('Updated note')
      );
  ...
  describe('Note Deletion', () => {
    ...
    test('should not delete system notes', async () => {
      await assert.rejects(async () => await notesAPI.deleteNote(systemNote.id), ValidationError);
    });

    test('should handle non-existent note', async () => {
      await assert.rejects(async () => await notesAPI.deleteNote(99999), APIError);
    });
    ...
    test('should record deletion in timeline', async () => {
      ...
      const task = await taskAPI.getTask(testTask.id);
      const deleteNote = task.recent_notes.find(
        note => note.note.includes('Deleted') && note.note.includes('comment')
      );
  ...
  describe('Activity Summary', () => {
    ...
    test('should count activities by type', async () => {
      ...
      const stateChangeActivity = summary.summary.activity_by_type.find(
        a => a.note_type === 'state_change'
      );
    ...
    test('should count activities by author', async () => {
      ...
      const user1Activity = summary.summary.activity_by_author.find(
        a => a.author === 'user1@example.com'
      );
  ...
  describe('Edge Cases', () => {
    test('should handle invalid task ID formats', async () => {
      ...
      await assert.rejects(async () => await notesAPI.getNotes('not-a-number'), ValidationError);
    });
