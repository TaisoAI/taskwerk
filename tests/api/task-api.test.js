...
describe('Task CRUD API', () => {
  ...
  describe('Task Creation', () => {
    ...
    test('should validate required fields', async () => {
      ...
      await assert.rejects(async () => await api.createTask(invalidTask), ValidationError);
    });
  ...
  describe('Task Retrieval', () => {
    ...
    test('should throw error for invalid string ID format', async () => {
      await assert.rejects(async () => await api.getTask('INVALID-ID'), Error);
    });
  ...
  describe('Task Updates', () => {
    ...
    test('should record changes in timeline', async () => {
      ...
      const statusNote = task.recent_notes.find(note => note.note.includes('Status changed'));
    ...
    test('should handle task not found', async () => {
      await assert.rejects(async () => await api.updateTask(99999, { name: 'Updated' }), APIError);
    });
  });
  ...
  describe('Task Deletion', () => {
    ...
    test('should handle task not found', async () => {
      await assert.rejects(async () => await api.deleteTask(99999), APIError);
    });
  ...
  describe('Transaction Safety', () => {
    test('should rollback on creation failure', async () => {
      ...
      api.transaction = async _operation => {
        // const _db = await api.getDatabase();
        throw new Error('Simulated database error');
      };

      await assert.rejects(async () => await api.createTask({ name: 'Test Task' }), Error);
    ...
    test('should rollback on update failure', async () => {
      ...
      await assert.rejects(async () => await api.updateTask(task.id, { name: 'Updated' }), Error);
