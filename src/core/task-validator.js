...
import { APIError, ValidationError } from '../api/base-api.js';
...
export class TaskValidator {
    ...
    validateTaskCompletion(taskData, errors = []) {
        ...
        const blockingDependencies = this.db.prepare(`
     ...
     */
    validateTaskAssignment(assignee, taskData = {}) {
        const errors = [];
        ...
        if (assignee) {
            const activeTaskCount = this.db.prepare(`
    ...
    validateDependencyCycle(taskId, dependsOnId) {
        ...
        const reverseDep = this.db.prepare(`
    ...
    findDependencyCycle(startTaskId, targetTaskId, visited = new Set(), path = []) {
        ...
        for (const dep of dependencies) {
            ...
            const cyclePath = this.findDependencyCycle(startTaskId, dep.depends_on_id, new Set(visited), [...path]);
            if (cyclePath.length > 0) {
                return cyclePath;
            }
        }
    ...
    validateTaskUpdate(existingTask, updates) {
        ...
        if (updates.status && updates.status !== existingTask.status) {
            this.validateStatusTransition(existingTask.status, updates.status, { ...existingTask, ...updates });
        }
