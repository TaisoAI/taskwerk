...
export class NotesAPI extends BaseAPI {
  ...
  async addNote(taskId, note, noteType = 'comment', context = {}) {
    ...
    if (!validateTaskId(taskId)) {
      throw new ValidationError('Invalid task ID format', [
        'Task ID must be a positive integer or string',
      ]);
    }
  ...
  async getNotes(taskId, options = {}) {
    ...
    if (!validOrderColumns.includes(orderBy)) {
      throw new ValidationError('Invalid order column', [
        `Must be one of: ${validOrderColumns.join(', ')}`,
      ]);
    }
  ...
  async updateNote(noteId, updates, context = {}) {
    ...
    return await this.transaction(db => {
      ...
      const validatedData = {
        ...
        note_type:
          updateData.note_type !== undefined ? updateData.note_type : existingNote.note_type,
  ...
  async deleteNote(noteId, context = {}) {
    ...
    return await this.transaction(db => {
      ...
      if (note.note_type === 'system') {
        throw new ValidationError('Cannot delete system notes', [
          'System-generated notes are part of the audit trail',
        ]);
      }
