...
describe('export command', () => {
  ...
  beforeEach(async () => {
    ...
    await api.addTaskNote('TASK-100', 'This needs review', 'user', 'Please check the implementation details');
  });
  ...
  it('should export to markdown by default', async () => {
    ...
    const output = testSetup.consoleLogSpy.mock.calls
      .find(call => call[0].includes('# Tasks Export'))?.[0];
  ...
  it('should export to CSV format', async () => {
    ...
    const output = testSetup.consoleLogSpy.mock.calls
      .find(call => call[0].includes('ID,Name,Description'))?.[0];
  ...
  it('should write to file when output specified', async () => {
    ...
    const successMessage = testSetup.consoleLogSpy.mock.calls
      .find(call => call[0].includes('✅ Exported'))?.[0];
  ...
  it('should include tags in markdown export', async () => {
    ...
    const output = testSetup.consoleLogSpy.mock.calls
      .find(call => call[0].includes('- Tags:'))?.[0];
  ...
  it('should include notes in markdown export', async () => {
    ...
    const output = testSetup.consoleLogSpy.mock.calls
      .find(call => call[0].includes('### Notes'))?.[0];
