...
describe('Relationship API', () => {
  ...
  describe('Dependency Management', () => {
    ...
    test('should prevent self-dependency', async () => {
      await assert.rejects(
        async () =>
          await relationshipAPI.addDependency(testTasks.child1.id, testTasks.child1.id, 'blocks'),
        ValidationError
      );
    });
    ...
    test('should detect circular dependencies', async () => {
      ...
      await assert.rejects(
        async () =>
          await relationshipAPI.addDependency(testTasks.child2.id, testTasks.child1.id, 'blocks'),
        ValidationError
      );
    });
    ...
    test('should handle non-existent tasks', async () => {
      await assert.rejects(
        async () => await relationshipAPI.addDependency(99999, testTasks.blocker.id, 'blocks'),
        APIError
      );

      await assert.rejects(
        async () => await relationshipAPI.addDependency(testTasks.child1.id, 99999, 'blocks'),
        APIError
      );
    });
  });
  ...
  describe('Subtask Management', () => {
    test('should add subtask relationship', async () => {
      const subtask = await relationshipAPI.addSubtask(testTasks.parent.id, testTasks.child1.id);
    ...
    test('should remove subtask relationship', async () => {
      ...
      const result = await relationshipAPI.removeSubtask(testTasks.parent.id, testTasks.child1.id);
    ...
    test('should move subtask to different parent', async () => {
      ...
      const result = await relationshipAPI.moveSubtask(testTasks.child1.id, testTasks.blocker.id);
  ...
  describe('Task Hierarchy', () => {
    ...
    test('should handle non-existent task', async () => {
      await assert.rejects(async () => await relationshipAPI.getTaskHierarchy(99999), APIError);
    });
  });
  ...
  describe('Error Handling', () => {
    test('should handle database transaction failures', async () => {
      ...
      relationshipAPI.transaction = async _operation => {
    ...
    test('should validate task existence before operations', async () => {
      await assert.rejects(async () => await relationshipAPI.getDependencies(99999), Error);
    });
  ...
  describe('Timeline Integration', () => {
    test('should record dependency changes in timeline', async () => {
      ...
      const dependencyNote = task.recent_notes.find(
        note => note.note.includes('dependency') && note.note.includes('Blocker Task')
      );
    ...
    test('should record dependency removal in timeline', async () => {
      await relationshipAPI.addDependency(testTasks.child1.id, testTasks.blocker.id, 'blocks');
      await relationshipAPI.removeDependency(testTasks.child1.id, testTasks.blocker.id, 'blocks', {
        user: 'test@example.com',
      });

      const task = await taskAPI.getTask(testTasks.child1.id);
      const removalNote = task.recent_notes.find(
        note => note.note.includes('Removed') && note.note.includes('dependency')
      );
    ...
    test('should record hierarchy changes in timeline', async () => {
      await relationshipAPI.addSubtask(testTasks.parent.id, testTasks.child1.id);
      // const _promoted = await relationshipAPI.promoteSubtask(
      await relationshipAPI.promoteSubtask(testTasks.child1.id, { user: 'test@example.com' });

      const task = await taskAPI.getTask(testTasks.child1.id);
      const promotionNote = task.recent_notes.find(
        note => note.note.includes('Promoted') && note.note.includes('independent')
      );
